...
const SAMLModuleManager = {
  ...
  passportSetup(passport, callback) {
    ...
    if (dbProvider) {
      ...
      logger.warn({}, 'SAML provider not configured (stored-only) — skipping passport strategy registration')
      return callback(null)
    }
    let samlOptions
    if (dbProvider) {
      ...
      samlOptions = {
        ...
        idpCert: dbProvider.idpCert || undefined, // stored PEM content (2026-08-29)
        privateKey: dbProvider.privateKey || undefined, // stored PEM content (2026-08-29)
        decryptionPvk: dbProvider.decryptionPvk || undefined, // stored PEM content (2026-08-29)
