...
const GitSyncMergeOverviewModal = ({
  ...
  const loadUnmergedCommits = () => {
    ...
    runAsync(getJSON(`/project/${projectId}/gitlab-sync/merge/overview`))
      ...
      <OLModalBody>
        ...
        {isSuccess && (
          <>
            ...
            <h3 className="gitlab-sync-commits-heading">
              {t('recent_commits_in_gitlab')}
            ...
            {data.commits?.length === 0 && (
              <div>
                <p className="small">{t('no_new_commits_in_gitlab')}</p>
              </div>
            )}
            ...
            {data.isProjectUpdated && (
              <>
                <hr />
                <OLForm>
                  <p>{t('sync_project_to_gitlab_explanation', { appName })}</p>
                  <OLRow>
                    <OLCol xs={12}>
                      <OLFormGroup>
                        <OLFormControl
                          ...
                          rows={1}
                          value={commitMessage}
                          placeholder={t('gitlab_commit_message_placeholder', { appName })}
