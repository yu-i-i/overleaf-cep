 ...
 *   1. GITLAB_CIPHER_PASSWORD || TOKEN_CIPHER_PASSWORD env var (explicit user config)
...
function _getEncryptorData() {
  const cipherPassword = process.env.GITLAB_TOKEN_CIPHER_PASSWORD || process.env.TOKEN_CIPHER_PASSWORD
  const cipherLabel = process.env.GITLAB_TOKEN_CIPHER_LABEL ||
  ...
  const cipherFile = process.env.GITLAB_TOKEN_CIPHER_FILE ||
