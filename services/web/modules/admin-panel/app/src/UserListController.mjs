...
async function registerNewUser(req, res, next) {
 ...
 let user
  ...
  const newUser = { id, email, firstName: first_name, lastName: last_name, isAdmin, signUpDate, lastActive, suspended, inactive: true, deleted: false, authMethods }
...
function _sortAndPaginate(users, sort, page) {
  if (
    (sort.by && !['lastLoggedIn', 'lastActive', 'signUpDate', 'email', 'name', 'deletedAt'].includes(sort.by)) ||
