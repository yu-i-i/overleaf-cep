...
export const extension = (options: Record<string, any>): Extension => {
   ...
   *  1. On mount — checks if LT is available on the server and enables checking if LT
  ...
  const syncPlugin = ViewPlugin.define(view => {
    ...
    // ── Initialisation ──────────────────────────────────────────────────
    ltServerAvailable = getMeta('ol-ExposedSettings').languageToolAvailable
    if (ltServerAvailable && getStoredEnabled(projectId)) {
      enabled = true
      scheduleCheck(500)
    }
