...
export default function LanguageToolLanguageSetting() {
  ...
  const [ltAvailable, setLtAvailable] = useState<boolean>(false)
  ...
  // ── Check availability ─────────────────────────────────────────────────────
  useEffect(() => {
    const { languageToolAvailable } = getMeta('ol-ExposedSettings')

    setLtAvailable(languageToolAvailable)
